...
// Handler provides request handler functions for the Role aggregate.
type Handler struct {
	repo       Repo
	client     *RoleClient
...
// "auth.role.v1.{Operation}" where {Operation} is:
...
// first request.
func NewHandler(repo Repo, client *RoleClient, authorizer authz.Authorizer) *Handler {
	if authorizer == nil {
		panic("rolev1.NewHandler: authorizer must not be nil (use allowall.Authorizer{} for no enforcement)")
	}
...
func (h *Handler) RegisterRoutes(router *protosource.Router) {
	router.Handle("POST", "auth/role/v1/create", h.HandleCreate)
	router.Handle("POST", "auth/role/v1/rename", h.HandleRename)
	router.Handle("POST", "auth/role/v1/setdescription", h.HandleSetDescription)
	router.Handle("POST", "auth/role/v1/addfunction", h.HandleAddFunction)
	...
	router.Handle("GET", "auth/role/v1/{id}", h.HandleGet)
	router.Handle("GET", "auth/role/v1/{id}/history", h.HandleHistory)
	router.Handle("GET", "auth/role/v1/query/by-state", h.HandleQueryByState)
}
...
func (h *Handler) HandleCreate(ctx context.Context, request protosource.Request) protosource.Response {
	ctx, err := h.authorizer.Authorize(ctx, request, "auth.role.v1.Create")
	if err != nil {
...
func (h *Handler) HandleRename(ctx context.Context, request protosource.Request) protosource.Response {
	ctx, err := h.authorizer.Authorize(ctx, request, "auth.role.v1.Rename")
	if err != nil {
		return authzErrorResponse(request, err)
	}
...
// HandleSetDescription processes a SetDescription command.
func (h *Handler) HandleSetDescription(ctx context.Context, request protosource.Request) protosource.Response {
	ctx, err := h.authorizer.Authorize(ctx, request, "auth.role.v1.SetDescription")
	if err != nil {
		return authzErrorResponse(request, err)
	}
...
// HandleAddFunction processes a AddFunction command.
func (h *Handler) HandleAddFunction(ctx context.Context, request protosource.Request) protosource.Response {
	ctx, err := h.authorizer.Authorize(ctx, request, "auth.role.v1.AddFunction")
	if err != nil {
		return authzErrorResponse(request, err)
	}
...
// HandleRemoveFunction processes a RemoveFunction command.
func (h *Handler) HandleRemoveFunction(ctx context.Context, request protosource.Request) protosource.Response {
	ctx, err := h.authorizer.Authorize(ctx, request, "auth.role.v1.RemoveFunction")
	if err != nil {
		return authzErrorResponse(request, err)
	}
...
func (h *Handler) HandleDelete(ctx context.Context, request protosource.Request) protosource.Response {
	ctx, err := h.authorizer.Authorize(ctx, request, "auth.role.v1.Delete")
	if err != nil {
		return authzErrorResponse(request, err)
	}
...
// HandleLoad retrieves the current state of a Role aggregate by
...
func (h *Handler) HandleLoad(ctx context.Context, request protosource.Request) protosource.Response {
	ctx, err := h.authorizer.Authorize(ctx, request, "auth.role.v1.LoadRole")
	if err != nil {
		return authzErrorResponse(request, err)
	}
...
// HandleGet retrieves the Role aggregate from the materialized store.
...
func (h *Handler) HandleGet(ctx context.Context, request protosource.Request) protosource.Response {
	ctx, err := h.authorizer.Authorize(ctx, request, "auth.role.v1.GetRole")
	if err != nil {
		return authzErrorResponse(request, err)
	}
	...
	aggregate, err := h.client.GetRole(ctx, id)
	if err != nil {
...
// HandleHistory retrieves the full event history for a Role aggregate.
func (h *Handler) HandleHistory(ctx context.Context, request protosource.Request) protosource.Response {
	ctx, err := h.authorizer.Authorize(ctx, request, "auth.role.v1.HistoryRole")
	if err != nil {
		return authzErrorResponse(request, err)
	}
...
func (h *Handler) HandleQueryByState(ctx context.Context, request protosource.Request) protosource.Response {
	ctx, err := h.authorizer.Authorize(ctx, request, "auth.role.v1.QueryByState")
	if err != nil {
		return authzErrorResponse(request, err)
	}
	...
	if skOp == "" {
		results, err := h.client.SelectRoleByState(ctx, state)
		if err != nil {
	...
	if op == opaquedata.Between {
		...
		results, err := h.client.SelectRoleByStateWithCreateAt(ctx, state, op, skVal, skVal2)
		if err != nil {
			return errorResponse(request, http.StatusInternalServerError, "QUERY_EXEC", "query failed", err)
		}
		return queryResponse(request, results)
	...
	results, err := h.client.SelectRoleByStateWithCreateAt(ctx, state, op, skVal)
	if err != nil {
		return errorResponse(request, http.StatusInternalServerError, "QUERY_EXEC", "query failed", err)
	}
	return queryResponse(request, results)
...
// queryResponse wraps query results in the RoleList message and
// serializes using content negotiation (protobuf or JSON based on Accept header).
func queryResponse(request protosource.Request, results []*Role) protosource.Response {
	list := &RoleList{Items: results}
