...
func (h *Handler) HandleQueryByEmail(ctx context.Context, request protosource.Request) protosource.Response {
	...
	if emailRaw == "" {
		return errorResponse(http.StatusBadRequest, "QUERY_MISSING_PK", "missing required parameter: email", nil)
	}
	email, emailErr := parseQueryParamString(emailRaw)
	if emailErr != nil {
		return errorResponse(http.StatusBadRequest, "QUERY_BAD_PARAM", fmt.Sprintf("invalid value for email: %v", emailErr), nil)
	}
