...
// "auth.token.v1.{Operation}" where {Operation} is:
...
func (h *Handler) RegisterRoutes(router *protosource.Router) {
	...
	router.Handle("GET", "auth/token/v1/load/{id}", h.HandleLoad)
	router.Handle("GET", "auth/token/v1/{id}", h.HandleGet)
	router.Handle("GET", "auth/token/v1/{id}/history", h.HandleHistory)
	router.Handle("GET", "auth/token/v1/query/by-state", h.HandleQueryByState)
}
...
// HandleLoad retrieves the current state of a Token aggregate by
...
func (h *Handler) HandleLoad(ctx context.Context, request protosource.Request) protosource.Response {
	ctx, err := h.authorizer.Authorize(ctx, request, "auth.token.v1.LoadToken")
	if err != nil {
		return authzErrorResponse(request, err)
	}
...
// HandleGet retrieves the Token aggregate from the materialized store.
...
func (h *Handler) HandleGet(ctx context.Context, request protosource.Request) protosource.Response {
	ctx, err := h.authorizer.Authorize(ctx, request, "auth.token.v1.GetToken")
	if err != nil {
		return authzErrorResponse(request, err)
	}
...
func (h *Handler) HandleHistory(ctx context.Context, request protosource.Request) protosource.Response {
	ctx, err := h.authorizer.Authorize(ctx, request, "auth.token.v1.HistoryToken")
	if err != nil {
		return authzErrorResponse(request, err)
	}
...
func (h *Handler) HandleQueryByState(ctx context.Context, request protosource.Request) protosource.Response {
	ctx, err := h.authorizer.Authorize(ctx, request, "auth.token.v1.QueryByState")
	if err != nil {
		return authzErrorResponse(request, err)
	}
	...
	if skOp == "" {
		results, err := h.client.SelectTokenByState(ctx, state)
		if err != nil {
	...
	if op == opaquedata.Between {
		...
		results, err := h.client.SelectTokenByStateWithCreateAt(ctx, state, op, skVal, skVal2)
		if err != nil {
			return errorResponse(request, http.StatusInternalServerError, "QUERY_EXEC", "query failed", err)
		}
		return queryResponse(request, results)
	...
	results, err := h.client.SelectTokenByStateWithCreateAt(ctx, state, op, skVal)
	if err != nil {
		return errorResponse(request, http.StatusInternalServerError, "QUERY_EXEC", "query failed", err)
	}
	return queryResponse(request, results)
