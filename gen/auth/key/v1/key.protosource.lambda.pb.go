...
// Handler provides request handler functions for the Key aggregate.
type Handler struct {
	repo       Repo
	client     *KeyClient
...
// first request.
func NewHandler(repo Repo, client *KeyClient, authorizer authz.Authorizer) *Handler {
	if authorizer == nil {
		panic("keyv1.NewHandler: authorizer must not be nil (use allowall.Authorizer{} for no enforcement)")
	}
...
func (h *Handler) RegisterRoutes(router *protosource.Router) {
	router.Handle("POST", "auth/key/v1/generate", h.HandleGenerate)
	router.Handle("POST", "auth/key/v1/retire", h.HandleRetire)
	...
	router.Handle("GET", "auth/key/v1/query/by-state", h.HandleQueryByState)
}

// HandleGenerate processes a Generate command.
func (h *Handler) HandleGenerate(ctx context.Context, request protosource.Request) protosource.Response {
	ctx, err := h.authorizer.Authorize(ctx, request, "auth.key.v1.Generate")
	if err != nil {
...
// HandleRetire processes a Retire command.
func (h *Handler) HandleRetire(ctx context.Context, request protosource.Request) protosource.Response {
	ctx, err := h.authorizer.Authorize(ctx, request, "auth.key.v1.Retire")
	if err != nil {
		return authzErrorResponse(request, err)
	}
...
// HandleExpire processes a Expire command.
func (h *Handler) HandleExpire(ctx context.Context, request protosource.Request) protosource.Response {
	ctx, err := h.authorizer.Authorize(ctx, request, "auth.key.v1.Expire")
	if err != nil {
		return authzErrorResponse(request, err)
	}
...
func (h *Handler) HandleQueryByState(ctx context.Context, request protosource.Request) protosource.Response {
	...
	if skOp == "" {
		results, err := h.client.SelectKeyByState(ctx, state)
		if err != nil {
	...
	if op == opaquedata.Between {
		...
		results, err := h.client.SelectKeyByStateWithCreateAt(ctx, state, op, skVal, skVal2)
		if err != nil {
			return errorResponse(request, http.StatusInternalServerError, "QUERY_EXEC", "query failed", err)
		}
		return queryResponse(request, results)
	...
	results, err := h.client.SelectKeyByStateWithCreateAt(ctx, state, op, skVal)
	if err != nil {
		return errorResponse(request, http.StatusInternalServerError, "QUERY_EXEC", "query failed", err)
	}
	return queryResponse(request, results)
...
// queryResponse wraps query results in the KeyList message and
// serializes using content negotiation (protobuf or JSON based on Accept header).
func queryResponse(request protosource.Request, results []*Key) protosource.Response {
	list := &KeyList{Items: results}
