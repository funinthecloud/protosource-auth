// Code generated by protoc-gen-protosource. client.gotext DO NOT EDIT.

package issuerv1

import (
	"context"
	"fmt"
	"strconv"

	historyv1 "github.com/funinthecloud/protosource/gen/history/v1"
	responsev1 "github.com/funinthecloud/protosource/gen/response/v1"
	"github.com/funinthecloud/protosource/httpclient"
)

const routePath = "auth/issuer/v1"

// HTTPClient is a typed HTTP client for the Issuer aggregate.
type HTTPClient struct {
	c httpclient.Doer
}

// NewHTTPClient creates a new Issuer HTTP client.
// Accepts httpclient.Doer for testability — use *httpclient.Client in
// production and a mock in tests.
func NewHTTPClient(c httpclient.Doer) *HTTPClient {
	return &HTTPClient{c: c}
}

// Register sends the Register command.
func (c *HTTPClient) Register(ctx context.Context, id string, iss string, displayName string, kind Kind, defaultAlgorithm string, jwksUrl string, oidc *OIDCConfig) (responsev1.Responseer, error) {
	cmd := &Register{
		Id:               id,
		Iss:              iss,
		DisplayName:      displayName,
		Kind:             kind,
		DefaultAlgorithm: defaultAlgorithm,
		JwksUrl:          jwksUrl,
		Oidc:             oidc,
	}
	return c.c.Apply(ctx, routePath, cmd)
}

// Rename sends the Rename command.
func (c *HTTPClient) Rename(ctx context.Context, id string, displayName string) (responsev1.Responseer, error) {
	cmd := &Rename{
		Id:          id,
		DisplayName: displayName,
	}
	return c.c.Apply(ctx, routePath, cmd)
}

// SetDefaultAlgorithm sends the SetDefaultAlgorithm command.
func (c *HTTPClient) SetDefaultAlgorithm(ctx context.Context, id string, defaultAlgorithm string) (responsev1.Responseer, error) {
	cmd := &SetDefaultAlgorithm{
		Id:               id,
		DefaultAlgorithm: defaultAlgorithm,
	}
	return c.c.Apply(ctx, routePath, cmd)
}

// SetJWKSURL sends the SetJWKSURL command.
func (c *HTTPClient) SetJWKSURL(ctx context.Context, id string, jwksUrl string) (responsev1.Responseer, error) {
	cmd := &SetJWKSURL{
		Id:      id,
		JwksUrl: jwksUrl,
	}
	return c.c.Apply(ctx, routePath, cmd)
}

// Deactivate sends the Deactivate command.
func (c *HTTPClient) Deactivate(ctx context.Context, id string) (responsev1.Responseer, error) {
	cmd := &Deactivate{
		Id: id,
	}
	return c.c.Apply(ctx, routePath, cmd)
}

// Reactivate sends the Reactivate command.
func (c *HTTPClient) Reactivate(ctx context.Context, id string) (responsev1.Responseer, error) {
	cmd := &Reactivate{
		Id: id,
	}
	return c.c.Apply(ctx, routePath, cmd)
}

// Delete sends the Delete command.
func (c *HTTPClient) Delete(ctx context.Context, id string) (responsev1.Responseer, error) {
	cmd := &Delete{
		Id: id,
	}
	return c.c.Apply(ctx, routePath, cmd)
}

// SetOIDCConfig sends the SetOIDCConfig command.
func (c *HTTPClient) SetOIDCConfig(ctx context.Context, id string, oidc *OIDCConfig) (responsev1.Responseer, error) {
	cmd := &SetOIDCConfig{
		Id:   id,
		Oidc: oidc,
	}
	return c.c.Apply(ctx, routePath, cmd)
}

// ClearOIDCConfig sends the ClearOIDCConfig command.
func (c *HTTPClient) ClearOIDCConfig(ctx context.Context, id string) (responsev1.Responseer, error) {
	cmd := &ClearOIDCConfig{
		Id: id,
	}
	return c.c.Apply(ctx, routePath, cmd)
}

// Load retrieves the current state of the Issuer aggregate via event replay.
func (c *HTTPClient) Load(ctx context.Context, id string) (*Issuer, error) {
	agg := &Issuer{}
	if err := c.c.Load(ctx, routePath, id, agg); err != nil {
		return nil, err
	}
	return agg, nil
}

// Get retrieves the Issuer aggregate from the materialized store.
func (c *HTTPClient) Get(ctx context.Context, id string) (*Issuer, error) {
	agg := &Issuer{}
	if err := c.c.Get(ctx, routePath, id, agg); err != nil {
		return nil, err
	}
	return agg, nil
}

// History retrieves the full event history for the Issuer aggregate.
func (c *HTTPClient) History(ctx context.Context, id string) (*historyv1.History, error) {
	return c.c.History(ctx, routePath, id)
}

// QueryByState queries by state via GSI2.
func (c *HTTPClient) QueryByState(ctx context.Context, state State) ([]*Issuer, error) {
	params := map[string]string{
		"state": strconv.FormatInt(int64(state), 10),
	}
	list := &IssuerList{}
	if err := c.c.Query(ctx, routePath, "by-state", params, list); err != nil {
		return nil, err
	}
	return list.Items, nil
}

// QueryByStateWithCreateAt queries with a sort key condition (eq, lt, le, gt, ge, begins_with).
// For between queries, use QueryByStateBetweenCreateAt instead.
func (c *HTTPClient) QueryByStateWithCreateAt(ctx context.Context, state State, skOp string, createAt int64) ([]*Issuer, error) {
	if skOp == "between" {
		return nil, fmt.Errorf("use QueryByStateBetweenCreateAt for between queries")
	}
	params := map[string]string{
		"state":     strconv.FormatInt(int64(state), 10),
		"sk_op":     skOp,
		"create_at": strconv.FormatInt(createAt, 10),
	}
	list := &IssuerList{}
	if err := c.c.Query(ctx, routePath, "by-state", params, list); err != nil {
		return nil, err
	}
	return list.Items, nil
}

// QueryByStateBetweenCreateAt queries with a between sort key condition (inclusive range).
func (c *HTTPClient) QueryByStateBetweenCreateAt(ctx context.Context, state State, createAtFrom int64, createAtTo int64) ([]*Issuer, error) {
	params := map[string]string{
		"state":      strconv.FormatInt(int64(state), 10),
		"sk_op":      "between",
		"create_at":  strconv.FormatInt(createAtFrom, 10),
		"create_at2": strconv.FormatInt(createAtTo, 10),
	}
	list := &IssuerList{}
	if err := c.c.Query(ctx, routePath, "by-state", params, list); err != nil {
		return nil, err
	}
	return list.Items, nil
}

// Ensure strconv and fmt are used.
var _ = strconv.FormatInt
var _ = fmt.Errorf
