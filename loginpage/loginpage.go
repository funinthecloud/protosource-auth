...
func (p *Page) handlePage(_ context.Context, req protosource.Request) protosource.Response {
	...
	if redirect != "" && !httputil.IsAllowedRedirect(redirect, httputil.ReqHost(req)) {
		redirect = ""
	}
...
func isSameOrigin(req protosource.Request) bool {
	host := httputil.ReqHost(req)
	hostDomain := httputil.RegistrableDomain(host)
	if hostDomain == "" {
		return false
	}

	// Prefer Origin (always present on POSTs from modern browsers).
	if origin := httputil.ReqHeader(req, "Origin"); origin != "" {
		return matchesRegistrableDomain(origin, hostDomain)
	}

	// Fall back to Referer.
	if referer := httputil.ReqHeader(req, "Referer"); referer != "" {
...
func matchesRegistrableDomain(rawURL, expectedDomain string) bool {
	...
	return httputil.RegistrableDomain(u.Host) == expectedDomain
}
